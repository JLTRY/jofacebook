...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		JOFacebook
	@subpackage		JofacebookModel.php
...
/**
 * Jofacebook List Model
 *
 * @since  1.6
 */
class JofacebookModel extends ListModel
{
	/**
    ...
    protected array $viewAccess = [
        'posts.access' => 'post.access',
        'post.access' => 'post.access',
        'posts.dashboard_list' => 'post.dashboard_list',
        'post.dashboard_add' => 'post.dashboard_add',
    ];
	...
	protected array $styles = [
		...
		'administrator/components/com_jofacebook/assets/css/dashboard.css'
 	];
	...
	protected function parseViewDefinition(string $view): ?array
	{
		...
		if (strpos($view, '||') !== false)
		{
			...
			if (count($parts) === 3)
			{
				...
				return [
					'type' => $type,
					'name' => 'COM_JOFACEBOOK_DASHBOARD_' . StringHelper::safe($name, 'U'),
					'url' => $url,
					'image' => "{$name}.{$type}",
					'alt' => $name,
					'viewName' => $name,
		...
		if (strpos($view, '.') !== false)
		{
			...
			if ($action)
			{
				if ($action === 'add')
				{
					$url = "index.php?option=com_jofacebook&view={$name}&layout=edit";
					$image = "{$name}_{$action}.{$type}";
					$alt = "{$name}&nbsp;{$action}";
					$name = 'COM_JOFACEBOOK_DASHBOARD_' .
			...
			{
				$url = "index.php?option=com_jofacebook&view={$name}";
				$image = "{$name}.{$type}";
				$alt = $name;
				$name = 'COM_JOFACEBOOK_DASHBOARD_' .
		...
		return [
			...
			'url' => "index.php?option=com_jofacebook&view={$view}",
			'image' => "{$view}.png",
			'alt' => $view,
			'viewName' => $view,
			'add' => false,
	...
	protected function hasAccessToView(string $viewName, bool $add): bool
	{
		...
		$dashboardAdd = isset($viewAccess["{$viewName}.dashboard_add"]) &&
					$this->user->authorise($viewAccess["{$viewName}.dashboard_add"], 'com_jofacebook');

		$dashboardList = isset($viewAccess["{$viewName}.dashboard_list"]) &&
					$this->user->authorise($viewAccess["{$viewName}.dashboard_list"], 'com_jofacebook');

		if ($add && StringHelper::check($accessAdd))
		{
			return $this->user->authorise($accessAdd, 'com_jofacebook') && $dashboardAdd;
		}

		if (StringHelper::check($accessTo))
		{
			return $this->user->authorise($accessTo, 'com_jofacebook') && $dashboardList;
		}
