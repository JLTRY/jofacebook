...
class PostsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        // From the jofacebook_item table
        $query->from($db->quoteName('#__jofacebook_post', 'a'));
        $query->join('LEFT', $db->quoteName('#__categories', 'c') . ' ON (' . $db->quoteName('a.category') . ' = ' . $db->quoteName('c.id') . ')');
        ...
        if (!$user->authorise('core.options', 'com_jofacebook'))
        {
    ...
    public function getExportData($pks, $user = null)
    {
        ...
        if (($pks_size = UtilitiesArrayHelper::check($pks)) !== false || 'bulk' === $pks)
        {
            ...
            // From the jofacebook_post table
            $query->from($db->quoteName('#__jofacebook_post', 'a'));
    ...
    */
    ...
    protected function checkInNow(): bool
    {
        ...
        $time = ComponentHelper::getParams('com_jofacebook')->get('check_in');
        ...
        if ($time)
        {
            ...
            $query = $db->getQuery(true);
            $query->select('*');
            $query->from($db->quoteName('#__jofacebook_post'));
            ...
            if ($db->getNumRows())
            {
                ...
                // Check table.
                $query->update($db->quoteName('#__jofacebook_post'))->set($fields)->where($conditions); 
