...
/**
 * Script File of Jofacebook Component
 *
 * @since  3.6
 */
class Com_JofacebookInstallerScript implements InstallerScriptInterface
{
	/**
	 * The CMS Application.
	 *
	...
	public function __construct()
	{
		...
		if (is_file(JPATH_ROOT . '/administrator/components/com_jofacebook/jofacebook.php'))
		{
			// remove Joomla 3 files
			$this->deleteFiles = [
				'/administrator/components/com_jofacebook/jofacebook.php',
				'/administrator/components/com_jofacebook/controller.php',
				'/components/com_jofacebook/jofacebook.php',
				'/components/com_jofacebook/controller.php',
				'/components/com_jofacebook/router.php',
			];
		}

		// check if the Folders exist
		if (is_dir(JPATH_ROOT . '/administrator/components/com_jofacebook/modules'))
		{
			// remove Joomla 3 folder
			$this->deleteFolders = [
				'/administrator/components/com_jofacebook/controllers',
				'/administrator/components/com_jofacebook/helpers',
				'/administrator/components/com_jofacebook/modules',
				'/administrator/components/com_jofacebook/tables',
				'/administrator/components/com_jofacebook/views',
				'/components/com_jofacebook/controllers',
				'/components/com_jofacebook/helpers',
				'/components/com_jofacebook/modules',
				'/components/com_jofacebook/views',
			];
		}
	}
	...
	protected function setActionLogsExtensions(): void
	{
		...
		$data = new stdClass();
		$data->extension = 'com_jofacebook';

		// Check if jofacebook action log extension is already in action logs extensions DB.
		$query = $this->db->getQuery(true);
		$query->select($this->db->quoteName(['id']));
		...
		if ($this->db->getNumRows())
		{
			...
			$this->app->enqueueMessage(
				Text::_('The (com_jofacebook) is already in the <b>#__action_logs_extensions</b> table.')
			);
		}
		elseif ($this->db->insertObject('#__action_logs_extensions', $data))
		{
			...
			$this->app->enqueueMessage(
				Text::_('The (com_jofacebook) was successfully added to the <b>#__action_logs_extensions</b> table.')
			);
		}
	}
	...
	protected function setAssetsRules(string $rules): void
	{
		// Condition.
		$conditions = [
			$this->db->quoteName('name') . ' = ' . $this->db->quote('com_jofacebook')
		];
		...
		if ($done)
		{
			// give a success message
			$this->app->enqueueMessage(
				Text::_('The (com_jofacebook) rules was successfully added to the <b>#__assets</b> table.')
			);
		}
	}
	...
	protected function setExtensionsParams(string $params): void
	{
		// Condition.
		$conditions = [
			$this->db->quoteName('element') . ' = ' . $this->db->quote('com_jofacebook')
		];
		...
		if ($done)
		{
			// give a success message
			$this->app->enqueueMessage(
				Text::_('The (com_jofacebook) params was successfully added to the <b>#__extensions</b> table.')
			);
		}
	}
	 ...
	 *
	 * @param   int     $accessWorseCase   This is the max rules column size com_jofacebook would needs.
	...
	protected function setDatabaseAssetsRulesFix(int $accessWorseCase, string $dataType): void
	{
		...
		if ($this->db->execute())
		{
			...
			if ($rule_length <= $accessWorseCase)
			{
				...
				$fix = "ALTER TABLE `#__assets` CHANGE `rules` `rules` {$dataType} NOT NULL COMMENT 'JSON encoded access control. Enlarged to {$dataType} by Jofacebook';";
	...
	{
		// Remove jofacebook view from the action_log_config table
		$condition = [
			$this->db->quoteName('type_alias') . ' = '. $this->db->quote($context)
		];
		...
		// Execute the query to remove com_jofacebook.view
		$done = $this->db->execute();
		if ($done)
		{
			// If successfully removed jofacebook view add queued success message.
			$this->app->enqueueMessage(
	...
	{
		// Remove jofacebook assets from the assets table
		$condition = [
			$this->db->quoteName('name') . ' LIKE ' . $this->db->quote('com_jofacebook.%')
		];
		...
		if ($done)
		{
			// If successfully removed jofacebook add queued success message.
			$this->app->enqueueMessage(
				Text::_('All related (com_jofacebook) items was removed from the <b>#__assets</b> table.')
			);
		}
	}
	...
	{
		// Remove jofacebook from the action_logs_extensions table
		$extension = [
			$this->db->quoteName('extension') . ' = ' . $this->db->quote('com_jofacebook')
		];
		...
		// Execute the query to remove jofacebook
		$done = $this->db->execute();
		if ($done)
		{
			// If successfully remove jofacebook add queued success message.
			$this->app->enqueueMessage(
				Text::_('The (com_jofacebook) extension was removed from the <b>#__action_logs_extensions</b> table.')
			);
		}
	}
