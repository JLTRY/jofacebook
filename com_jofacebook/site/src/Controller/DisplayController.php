...
/------------------------------------------------------------------------------------------------------*/
namespace JLTRY\Component\Jofacebook\Site\Controller;
...
/**
 * Jofacebook master site display controller.
 *
 * @since   4.0
 */
class DisplayController extends BaseController
{
    ...
    function display($cachable = false, $urlparams = false)
    {
        ...
        if ($isEdit && !$this->checkEditId($view, $id))
        {
            ...
            if ($refid > 0 && StringHelper::check($ref))
            {
                ...
                $this->setRedirect(Route::_('index.php?option=com_jofacebook&view=' . (string) $ref . '&layout=edit&id=' . (int) $refid, false));
            }
            elseif (StringHelper::check($ref))
            {
                // redirect to ref
                 $this->setRedirect(Route::_('index.php?option=com_jofacebook&view=' . (string) $ref, false));
            }
            else
            {
                // normal redirect back to the list default site view
                $this->setRedirect(Route::_('index.php?option=com_jofacebook&view=', false));
            }
    ...
    {
        if (parent::checkEditId("com_jofacebook.edit.{$context}", $id))
        {
            return true;
        }
        ...
        {
            $this->holdEditId("com_jofacebook.edit.{$context}", $id);
    ...
    protected function canEditId($view, $id): bool
    {
        ...
        {
            $access = ($this->identity->authorise($allowed['access'], "com_jofacebook.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['access'], 'com_jofacebook'));
        }
        ...
        {
            $edit = ($this->identity->authorise($allowed['edit'], "com_jofacebook.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['edit'], 'com_jofacebook'));
        }

        // check if this item can be edited by owner
        if ($access && !$edit && isset($allowed['edit.own']))
        {
            $edit = ($this->identity->authorise($allowed['edit.own'], "com_jofacebook.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['edit.own'], 'com_jofacebook'));
        }
